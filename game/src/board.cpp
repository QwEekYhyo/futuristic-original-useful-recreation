...
player board::get_winner() const {
  ...
  int min = x - starting_index1 <= y - starting_index2 ? x - starting_index1
                                                       : y - starting_index2;
  diagonal_start = {x - min, y - min};
  min = ending_index1 - diagonal_start.at(0) <=
                ending_index2 - diagonal_start.at(1)
            ? ending_index1 - diagonal_start.at(0)
            : ending_index2 - diagonal_start.at(1);

  for (int i = 0; i <= min; i++) {
    for (int j = 0; j < 4; j++) {
      segment.at(j) = {diagonal_start.at(0) + i + j,
                       diagonal_start.at(1) + i + j};
    }
    winner = get_winner_from_arr(segment);
    if (winner != player::none) {
      return winner;
  ...
  min = 3 + ending_index1 - x <= y - starting_index2 ? 3 + ending_index1 - x
                                                     : y - starting_index2;
  diagonal_start = {x + min, y - min};
  min = diagonal_start.at(0) - starting_index1 <=
                ending_index2 - diagonal_start.at(1)
            ? diagonal_start.at(0) - starting_index1
            : ending_index2 - diagonal_start.at(1);
...
pair<int> minimax(const board &b, int depth, int alpha, int beta,
  ...
  if (depth == 0 || winner != player::none) {
    if (winner == player::two) {
      ...
      return {-1, b.evaluate_position(player::two)};
    }
  }
